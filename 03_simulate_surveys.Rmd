...
NOTE: this file takes about 30 minutes to run on a 2020 Macbook Pro with 8 cores.
...
#target.num.sibships <- 5000
target.num.sibships <- 20000

## number of repetitions for each combination of simulation parameters
#M <- 3
M <- 1000
...
obs.sibs <- obs.sibs %>%
  ...
  mutate(alive = case_when(died.past7yr == 1 ~ 0L,
                           lived.past7yr == 0 ~ 0L,
...
pseudou.sibships %>% slice_head(n=100)
```
...
ego.frame.census <- ego.frame.census %>%
  select(-starts_with('v.')) %>%
  #rename_(.dots=setNames(names(.), gsub("y\\.", "d.", names(.))))
  rename(!!!setNames(names(.), gsub("y\\.", "d.", names(.))))
...
## M: number of repetitions for each combination of parameters
# (now set at the top of the file)
